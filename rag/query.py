...
先把结果介绍清楚，再随口点评，不要只丢一个名称。
介绍：名称、它是什么，以及资料里真正有用的效果/获取。
点评：可以吐槽、起哄、脑补船长拿去干什么，但每条换个说法，不要写成评测表，也不要每条都问要不要再来一次。
Markdown 点到为止。名称用 **中文名(English Name)**。游戏事实只能来自参考资料；不要列来源，不要提没抽中的词条。
"""
